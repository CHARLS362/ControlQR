...
export async function POST(request: Request) {
  try {
    ...
    const externalApiUrl = 'http://31.97.169.107:8093/api/grado/registrar';
    ...
    if (!response.ok || responseData.success !== 1) {
      ...
      const errorMessage = responseData.message || 'Error al registrar el grado.';
    ...
    console.error('Error en /api/grades/register:', error);
