...
export async function POST(request: Request) {
  try {
    ...
    const externalApiUrl = 'http://31.97.169.107:8093/api/seccion/registrar';
    ...
    if (!response.ok || responseData.success !== 1) {
      ...
      const errorMessage = responseData.message || 'Error al registrar la sección.';
    ...
    console.error('Error en /api/sections/register:', error);
